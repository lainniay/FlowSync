...
| `TASK_ASSIGNEE_CHANGED` | 409 | 修改任务状态期间负责人发生并发变化，客户端应刷新后重试 |
...
| Profile | `GET /users/{userId}/profile` | 路径参数 `userId` | 登录用户 | 200：公开用户资料 |
| Users | `GET /users` | 查询参数见 5.1 | ADMIN | 200：User 分页 |
| Users | `GET /user-options` | 可选 `q` 用户名 | 登录用户 | 200：用户选择项数组 |
...
| Overview | `GET /admin/overview` | 无 | ADMIN | 200：AdminOverview |
...
缺少 `password`、发送 `null` 或空字符串时返回 `422 VALIDATION_ERROR`；超过 72 UTF-8 字节或凭据不匹配时统一返回 `401 INVALID_CREDENTIALS`。
...
| `phone` | string/null | 是 | `null` 或 1 到 20；空字符串无效 |
...
任意登录用户可查看其他用户的协作联系方式。成功响应包含 `username`、`displayName`、
`phone`、`email`、`systemRole` 和 `active`，不返回用户 ID、密码信息或账号时间字段。
用户 ID 只作为路由参数使用，不在其他用户资料界面展示。目标用户不存在时返回 `404`。
...
`GET /user-options?q=zhang`

任意登录用户可查询有效的 `USER` 账号。`q` 可省略；提供时仅按用户名模糊匹配。
响应只包含选择器内部关联值和用户名，不返回展示名或联系方式：
...
| `userId` | string | 只返回该 USER 参与的项目；与当前登录用户可见范围取交集 |
| `myRole` | `OWNER \| MEMBER` | 按当前登录 USER 的参与角色筛选；`MEMBER` 不包含其负责的项目，ADMIN 使用时返回空列表 |
...
`GET /projects/{projectId}/invitation-candidates?q=zhang`

`q` 必须为 2 到 100 个字符。仅当前未归档项目的 owner 可以查询；最多返回 20 个
匹配展示名或用户名的有效 USER，并排除现有成员和已有 `PENDING` 邀请的用户。
响应仅包含选择器所需字段：
...
界面必须展示名称和用户名，`id` 仅作为选择器内部值和写请求关联键，不得要求用户
手工输入或在普通业务界面展示。
...
| `incomplete` | boolean | `true` 时排除 `COMPLETED` 和 `CANCELLED` 任务；默认不限制 |
| `q` | string | 按标题模糊搜索 |
...
owner 或当前 assignee 可以修改，成功返回更新后的 `200 Task`。该接口只修改状态，因此不要求发送其他任务字段。若处理期间负责人持续发生并发变化，后端返回 `409 TASK_ASSIGNEE_CHANGED`，客户端应刷新任务后重试。
...
`projectId` 可省略。普通用户的工作台只包含个人相关数据：项目范围为自己参与的未归档项目，
任务统计只包含自己负责的任务，最近活动只包含自己创建、负责或操作的记录。项目健康度仍反映
这些参与项目的整体情况。指定已归档或不可见的 `projectId` 返回 `404`。

ADMIN 使用独立的 `GET /admin/overview` 系统工作台；`GET /overview` 对 ADMIN 仍返回空个人总览。

响应字段：

| 响应字段 | 类型 | 可为 null | 说明 |
| --- | --- | --- | --- |
| `counts.projects` | integer | 否 | 未归档可见项目数 |
| `counts.inProgressProjects` | integer | 否 | 进行中的未归档可见项目数 |
| `counts.tasks` | integer | 否 | 当前用户负责的任务数 |
| `counts.completedTasks` | integer | 否 | 当前用户负责且已完成的任务数 |
| `counts.overdueTasks` | integer | 否 | 当前用户负责、已逾期且未完成的任务数 |
| `counts.blockedTasks` | integer | 否 | 当前用户负责的阻塞任务数 |
| `counts.dueSoonTasks` | integer | 否 | 当前用户负责、明日起 7 天内截止且未完成的任务数 |
| `counts.myOverdueTasks` | integer | 否 | 当前用户负责的逾期任务数 |
| `counts.myBlockedTasks` | integer | 否 | 当前用户负责的阻塞任务数 |
| `counts.myTodayDueTasks` | integer | 否 | 当前用户负责且今日截止的未完成任务数 |
| `counts.staleBlockedTasks` | integer | 否 | 更新时间超过 3 天的阻塞任务数 |
...
| `projectHealth` | array | 否 | 参与项目健康度；当前用户为 Owner 的项目优先，再按逾期数、阻塞数降序排列 |
| `projectHealth[].id` | string | 否 | 项目 ID |
| `projectHealth[].name` | string | 否 | 项目名称 |
| `projectHealth[].isOwner` | boolean | 否 | 当前用户是否为项目 Owner |
| `projectHealth[].status` | ProjectStatus | 否 | 项目状态 |
| `projectHealth[].endDate` | date | 是 | 项目截止日期 |
| `projectHealth[].tasks` | integer | 否 | 项目任务总数 |
| `projectHealth[].completedTasks` | integer | 否 | 项目已完成任务数 |
| `projectHealth[].overdueTasks` | integer | 否 | 项目逾期任务数 |
| `projectHealth[].blockedTasks` | integer | 否 | 项目阻塞任务数 |
| `recentActivities` | array | 否 | 与当前用户相关的最近活动，最多 10 条 |
...
仅 ADMIN 可访问。统计范围为全部用户及所有未归档项目中的任务，不包含归档项目内容。

响应包含：

- `counts`：启用/停用用户、USER/ADMIN、未归档项目、进行中项目、任务、已完成任务、
  逾期任务及涉及逾期任务的项目数。
- `projectsByStatus`、`tasksByStatus`：项目与任务状态分布。
- `focusProjects`：最多 10 个重点关注项目，优先按逾期任务数、阻塞任务数和截止日期排序；
  每项包含项目、Owner、任务进度、逾期/阻塞数及截止日期。
- `recentActivities`：全部未归档项目中的最近 10 条活动。
...
后端通过可配置的 OpenAI-compatible Provider 生成内容。当前兼容范围为 Bearer Token、
`/v1/chat/completions`、纯文本 message content 和非流式响应。Provider 调用失败、超时、
响应过大、响应无法解析或输出不符合本节约束时返回 `502 AI_PROVIDER_ERROR`；Provider 或
本服务的 AI 并发限制被触发时返回 `429 RATE_LIMITED`；AI 功能未启用时返回
`503 SERVICE_UNAVAILABLE`。这些错误都不能包含 Provider 原始响应、凭据或底层异常消息。
...
| `focus` | string/null | 否 | 最长 2000；可包含实际进展并要求生成 TaskLog 草稿 |
...
| 响应字段 | 类型 | 可为 null | 说明 |
| --- | --- | --- | --- |
| `suggestion` | string | 否 | AI 生成的任务建议，1 到 5000 |
| `generatedAt` | datetime | 否 | 后端成功解析并校验建议后的 UTC 时间 |
...
| 请求字段 | 类型 | 必填 | 规则 |
| --- | --- | --- | --- |
| `goal` | string | 是 | 1 到 500，不允许为空 |
| `description` | string/null | 否 | 最长 5000；补充背景 |
...
| 响应字段 | 类型 | 可为 null | 说明 |
| --- | --- | --- | --- |
| `overview` | string | 否 | 计划整体说明，1 到 2000 |
| `items` | AiTaskPlanItem[] | 否 | 1 到 `constraints.maxItems` 项，且最多 20 项 |
| `items[].draftId` | string | 否 | 1 到 100；本次计划内唯一的临时 ID |
| `items[].parentDraftId` | string | 是 | 最长 100；父条目的临时 ID，必须存在且不能形成循环 |
| `items[].title` | string | 否 | 1 到 100；任务标题 |
| `items[].description` | string | 是 | 最长 5000；任务描述 |
| `items[].priority` | Priority | 否 | 任务优先级 |
| `items[].dueDate` | date | 是 | 截止日期，非空时符合项目日期范围 |
| `items[].assigneeId` | string | 是 | 负责人 ID，非空时必须是提供给 Provider 的有效 USER 成员 |
| `generatedAt` | datetime | 否 | 后端成功解析并校验计划后的 UTC 时间 |
...
| `items[].draftId` | string | 是 | 1 到 100；本次请求内唯一 |
| `items[].parentDraftId` | string/null | 是 | 最长 100；只能指向本次请求中的条目且不能形成循环 |
