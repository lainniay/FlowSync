...
- An archived Project is read-only except for restoration and permanent deletion. Member, invitation, task, log, and summary writes must fail with `409 PROJECT_ARCHIVED`; invitation acceptance, rejection, and cancellation must leave the invitation unchanged.
