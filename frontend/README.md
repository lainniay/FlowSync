...
前端公共基座与业务模块复用同一套布局、路由、请求、错误处理、类型和设计规范。
...
| 工作台          | `src/views/overview/`                            | 已接入 Overview 类型、API、反馈状态和页面                              |
| 公共反馈页      | `src/views/ForbiddenView.vue`, `NotFoundView.vue` | 已有登录后 403 和兜底 404                                     |
| 模块路由         | `src/router/routes/`                              | 项目、管理、邀请、个人中心、任务、总结和 AI 子路由已接入根路由                    |
| 项目列表纵向示例   | `src/views/projects/`                             | 已有类型、API、查询、筛选、分页和完整反馈状态，可复制为新模块模板                    |
| 任务与内容协作模块 | `src/views/tasks/`, `summaries/`, `ai/`           | 已实现任务、日志、总结、AI 建议和 AI 计划页面                              |
...
frontend/
  ...
  src/
    ...
    router/
      ...
      routes/
        admin.ts                ADMIN 用户管理子路由
        ai.ts                   AI 任务计划子路由
        invitations.ts          USER 邀请子路由
        overview.ts             工作台子路由
        profile.ts              个人中心子路由
        projects.ts             项目子路由
        summaries.ts            总结子路由
    ...
    views/
      LoginView.vue             登录页
      overview/                 工作台页面、类型与 API
      admin/                    ADMIN 用户管理页面、类型与 API
      invitations/              USER 邀请页面、类型与 API
      profile/                  个人中心页面、类型与 API
      tasks/                    任务与任务日志页面、类型与 API
      summaries/                总结页面、类型与 API
      ai/                       AI 任务计划页面、类型与 API
...
公共基座与管理项目模块已经合并到 `main`。后续集成修复必须从最新 `main` 创建独立分支和
worktree，不得在共享仓库切换分支，也不得直接覆盖原业务 PR 分支。
...
- 集成修复分支：`codex/frontend-work-content-integration`
- 主要目录：`views/tasks/`、`views/summaries/`、`views/ai/`
- 模块开发文档：`frontend/B_WORK_CONTENT_DEVELOPMENT.md`
