...
- [ ] 完成 ADMIN 用户列表、搜索、筛选和分页。
- [ ] 完成用户创建、完整编辑、启停和管理员重置密码。
- [ ] 完成项目详情以及项目创建、完整编辑和 owner 转移。
- [ ] 完成项目归档、恢复和归档后的永久删除。
- [ ] 完成项目成员列表、ADMIN 添加成员和授权用户移除成员。
- [ ] 完成项目邀请列表、owner 发起邀请和授权用户取消邀请。
- [ ] 完成个人资料修改和当前用户修改密码。
- [ ] 为上述页面补充模块路由导出和最小组件测试。

#### Optimal

- [ ] 将工作台占位页接入真实 Overview 类型、API 和反馈状态。
- [ ] 完成 USER 收到的邀请列表以及接受、拒绝操作。
...
- `pnpm lint`：Oxlint 0 warning、0 error，ESLint 通过。
- `pnpm test`：11 个测试文件、113 个测试全部通过。
- `pnpm test:mock`：5 个测试文件、89 个 Mock 契约与状态测试全部通过。
- `pnpm build`：TypeScript 类型检查与 Vite production build 通过，共转换 1715 个模块。

## 更新文档

- 业务开发者在自己的模块开发文档中更新进度，不直接修改本 README。
- 基座维护者在 Review 或合并后更新本 README 的现成能力、任务状态和最终验证记录。
- 跨前后端契约发生变化时先更新 `../docs/api.md`，再更新 Mock、后端实现和前端类型。
- 尚未具备前置条件的任务在模块开发文档中写明依赖，不用空文件伪装成已完成。
