...
当前只确认 AppLayout、认证状态和子路由已经连接。真实统计数据将在工作台模块中通过
`GET /api/overview` 接入。
...
页面结果是 AppLayout 和 HomeView 同时存在：AppLayout 负责外框，HomeView 出现在
...
| `/overview` | App → AppLayout → HomeView | 已登录 | 基座页已实现，业务统计待接入 |
| `/403` | App → AppLayout → ForbiddenView | 已登录 | 已实现 |
| 未匹配地址 | App → AppLayout → NotFoundView | 已登录 | 已实现 |

用户管理、项目、任务、总结、邀请和个人中心菜单目前禁用。对应业务模块存在后，由基座维护者
接入正式路由并启用菜单；不要为了点亮菜单创建空页面。
